<!DOCTYPE html>
<html lang="en">

<head>
    <title>User Login</title>
    <!-- @vite(['resources/css/app.css', 'resources/js/app.js']) -->
<link rel="stylesheet" href="/build/assets/app-8f8302ad.css">
<script src="/build/assets/app-3720562b.js" defer></script>

</head>

<body>

    <x-user-navbar></x-user-navbar>
    <div class=" bg-gray-100 flex justify-center items-center min-h-screen ">
        <div class=" bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm ">
            <h2 class=" text-2xl text-gray-800 text-center mb-6">Forgot Password</h2>
            @error('user')
            <div class=" text-red-500 ">{{ $message }}</div>
            @enderror
            <form action="user-forgot-password" method="POST" class=" space-y-4">
                @csrf

                
                <div>
                    
                    <input type="text" id="email" placeholder="Enter User Email"
                        class=" w-full border px-4 border-gray-300 rounded-xl py-2 focus:outline-none" name="email">
                    @error('email')
                    <div class=" text-red-500 ">{{ $message }}</div>
                    @enderror
                </div>
                 
                <button type="submit" class=" cursor-pointer w-full bg-green-500 rounded-xl py-2 px-4 text-white font-bold ">Submit</button>
           
            </form>
        </div>
    </div>
</body>

</html>